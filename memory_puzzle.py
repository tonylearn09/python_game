...
assert (BROADWIDTH * BROADHEIGHT) % 2 == 0, \
    'Board needs to have an even nummber of boxes ' + 'for pairs of matches.'
...
assert len(ALLCOLORS) * len(ALLSHPAES) * 2 >= BROADWIDTH * BROADHEIGHT, \
    'Board is too big' + ' for the numbers of color/shape difined'
    ...
    while True:
        ...
        if boxx is not None and boxy is not None:
            ...
            if not revealed_boxes[boxx][boxy]:
                # Draw a blue highlight around the box
                # to let user know he/she has select
            ...
            if not revealed_boxes[boxx][boxy] and mouse_clicked:
                ...
                if first_selection is None:
                    ...
                    icon1shape, icon1color = \
                        getShapeAndColor(main_board, first_selection[0],
                                         first_selection[1])
                    icon2shape, icon2color = \
                        getShapeAndColor(main_board, boxx, boxy)

                    if icon1shape != icon2shape or icon1color != icon2color:
                        # Icons don't match. Re-cover up both sections
                        pygame.time.wait(1000)  # 1000ms = 1s
                        coverBoxAnimation(main_board,
                                          [(first_selection[0],
                                            first_selection[1]),
                                           (boxx, boxy)])
                        revealed_boxes[first_selection[0]][first_selection[1]] \
                            = False
...
def getRandomizedBoard():
    ...
    numIconsUsed = int(BROADWIDTH * BROADHEIGHT / 2)
